...
from torch_audiomentations import Shift, LowPassFilter, Gain, HighPassFilter, PitchShift
...
def pad_audio(w, mode="reflect", constant_value = 0):
    if mode != "constant":
        constant_value = 0

    def inner(audio_t):
        return F.pad(audio_t, [w] * 2, mode=mode, value=constant_value,)
...
def normalize_audio():
    def inner(audio_t):
        return (audio_t - audio_t.mean(-1, keepdim=True)) / audio_t.std(-1, keepdim=True)
    return inner
...
standard_transforms_audio = [
    # pad_audio(120000, mode="constant", constant_value=0.001),
    jitter_audio(0.3),
    random_scale_audio(),
    jitter_audio(0.3),
    # low_pass_audio(),
