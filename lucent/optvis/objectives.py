...
from lucent.optvis.objectives_util import _make_arg_str, _extract_act_pos, _T_handle_batch, _extract_act_pos_1D
...
def neuron_1D(layer, n_channel, x=None, batch=None):
    """Visualize a single neuron of a single channel.
    ...
    def inner(model):
        layer_t = model(layer)
        layer_t = _extract_act_pos_1D(layer_t, x)
        return -layer_t[:, n_channel].mean()
    return inner
